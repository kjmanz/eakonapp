...
- **畳数選択** - 6畳/8畳/10畳/12畳/14畳/18畳/20畳/23畳/26畳/29畳から選択
...
### 本体価格の更新
本体価格は画面上で入力します。標準値を持たせる場合は、入力フォーム側の初期値を変更してください。
...
パナソニック公式仕様に基づいて `model`、`coolW`（冷房消費電力）、`heatW`（暖房消費電力）、`apf`、`energySavingRate`（2027年度省エネ基準達成率）、`periodKWh`（期間消費電力量）、`lowTempHeatingKw`（外気温2℃時の暖房能力）を更新できます。
